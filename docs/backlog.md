...
## v0.5 端到端协作测试反馈（2026-06-01 kisara 实测）

> 详细文档：`docs/plans/2026-06-01-ux-critical-issues.md`

### 🔴 Agent 工作过程完全不可见
- **发现于：** 2026-06-01，kisara 首次 5 agent 协作测试
- **问题：** agent 启动后，用户完全不知道它在做什么。不像终端能实时看到思考、工具调用、文件读写。agent 是黑盒——启动了，然后要么很久后发消息，要么死了
- **影响：** 无法判断 agent 是正常工作还是卡死，无法信任 agent 自主工作
- **建议修复：** 把 SDK 事件流（tool_use/text/thinking）实时推送到前端，显示人类可读的活动描述
- **状态：** open
- **计划版本：** v0.6

### 🔴 Agent 状态显示不准确
- **发现于：** 2026-06-01，kisara 实测
- **问题：** agent 正在工作 → 前端显示"启动中"；agent 死了 → 显示"Dormant"。用户无法区分 working/idle/completed/error
- **影响：** 状态指示器完全误导用户
- **建议修复：** 细化状态为 spawning/working/idle/completed/error/stopped，前端实时刷新
- **状态：** open
- **计划版本：** v0.6

### 🔴 Agent 完成后无通知
- **发现于：** 2026-06-01，kisara 实测
- **问题：** agent 干完活，用户完全不知道。没有 toast、没有红点、没有声音。必须手动刷新
- **影响：** 用户不能离开页面，必须一直盯着
...
### 🔴 任务编排形同虚设
- **发现于：** 2026-06-01，kisara 实测
- **问题：** 创建任务后主 agent 不会自动认领；必须手动分配；分配后无法对话；没有反馈回路
...
- **影响：** 所有 agent 都是一样的配置，无法差异化
- **建议修复：** AgentPage 增加配置标签页，支持 system prompt、工具列表、MCP 配置、模型参数
- **状态：** open
- **计划版本：** v0.6

### 🟡 工作流页面无实际价值
- **发现于：** 2026-06-01，kisara 实测
- **问题：** 只显示原始元数据（agent 启动/死了），对用户毫无帮助。应该显示概括性信息
- **影响：** 页面占空间但无信息量
- **建议修复：** 聚合生成 agent 工作摘要、任务进度、Token 消耗、错误汇总
- **状态：** open
- **计划版本：** v0.6
...
- **影响：** 广播功能不可靠
- **建议修复：** 排查 wake 逻辑 + 前端显示 wake 结果 + 考虑 @everyone 指令
- **状态：** open
- **计划版本：** v0.6

### 🟡 无法中途干预/对话 agent
- **发现于：** 2026-06-01，kisara 实测
- **问题：** agent 启动后无法发消息、追加指令、修改需求。只能等完成或手动停止
- **影响：** 用户对 agent 完全失去控制
