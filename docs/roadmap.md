...
| **环 2** | 1.5 周 | Agent 生命周期 | Runtime 骨架 + spawn/stop + GUI | GUI 操作 spawn/stop，agent 在 Room 对话 | ⚠️ 部分完成：stale Runtime 假成功已修；Runtime runner 仍未兑现 SDK query/resume |
| **环 3** | 1 周 | 实时消息 + 唤醒 | Tool boundary 注入 + @mention 唤醒 + broadcast wake | agent 边干活边收到消息 | ⚠️ 部分完成：callback contract / dormant wake / broadcast-wake 已修；tool boundary 注入仍未兑现 |
| **环 4** | 1 周 | 跨机器 | 多 Runtime + Streamable HTTP MCP | 两台机器各跑一个 Runtime | ✅ |
| **环 5** | 1.5 周 | Harness 任务系统 | 状态机 + 任务派发 + 任务看板 + 任务分配通知 + 超时重试 | 创建任务 → 派给 agent → 完成 → 看板更新 | ✅ |
| **环 6** | 1 周 | Token 控制 + 配置 | Token 预算 + 配置编辑器 + Skills/MCP 选择器 | 编辑 agent 配置，看到 token 消耗 | ✅ |

> 2026-05-18 codex 验收补充：kisara 实测的 runtime 重复注册/假启动、DM internal error、空 room/拉 agent 入房、v2 agent 串旧版问题已修复并补测试。环 2/3 仍不能完全按 proposal 交付，原因是 Runtime runner 仍使用 Claude CLI child process，尚未实现 Agent SDK `query()` / resume / tool boundary 注入。
